...
export class GeoFeaturePlugin extends UIPlugin {
  ...
  geoFeatureNameToId(region: string, featureName: string): string | undefined {
    ...
    const cachedFeatureId = this.featureNameToIdCache[region][featureName];
    ...
    this.promises.push(
      new Promise<void>(async (resolve) => {
        const code = (await this.geoJsonService?.geoFeatureNameToId(region, featureName)) ?? null;
        this.featureNameToIdCache[region][featureName] = code;
        resolve();
      })
    );
    this.triggerFetching();
